...
class Camera1 extends CameraViewImpl implements Camera.PreviewCallback {
    ...
    private OneShotPreviewCallback mPreviewCallback;
    ...
    @Override
    void requestOneShotPreview(final OneShotPreviewCallback callback) {
        if (isCameraOpened()) {
            mPreviewCallback = callback;
            mCamera.setOneShotPreviewCallback(callback == null ? null : this);
        }
        else {
            mPreviewCallback = null;
            Log.w(Camera1.class.getSimpleName(), "Camera is closed !");
        }
    }
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        //已经关闭了的状态下来了这个回调应该继续回调吗？（不应该
        if (mPreviewCallback != null && previewSize != null) {
            mPreviewCallback.onShot(data, previewSize.getWidth(), previewSize.getHeight());
        }
    }
